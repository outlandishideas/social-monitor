...
class Metric_LinkedinEngagementLeveled extends Metric_Abstract {
    ...
    {
        $target = [];
        $target[1] = floatval(BaseController::getOption('in_engagement_target_level_1'));
        $target[2] = floatval(BaseController::getOption('in_engagement_target_level_2'));
        $target[3] = floatval(BaseController::getOption('in_engagement_target_level_3'));
        $target[4] = floatval(BaseController::getOption('in_engagement_target_level_4'));
        $target[5] = floatval(BaseController::getOption('in_engagement_target_level_5'));

        return $target;
    }

    /**
     * Gets the average Linkedin engagement over the given range
    ...
    {
        $data = $presence->getHistoricData($start, $end, 'instagram_engagement');
        $total = 0;
        $count = 0;
        foreach ($data as $d) {
            $total += $d['value'];
